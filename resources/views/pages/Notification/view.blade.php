    ...
    <main class="content">
        <div class="article-container">
            <div class="article-header">
                <h1 class="article-title">{{ $notification->tieu_de }}</h1>
                <div class="article-meta">
                    Loại thông báo:
                    <span style="color: {{ $notification->loai_thong_bao === 'Cảnh báo khẩn cấp' ? 'red' : '#0066cc' }}">
                        {{ $notification->loai_thong_bao }}
                    </span>
                    | Ngày đăng: {{ $notification->created_at->format('d/m/Y H:i') }}
                </div>
            </div>
            ...
            <div class="card-footer">
                @if($notification->loai_thong_bao === 'Yêu cầu đăng ký')
                    <form action="{{ route('thongbao.approve', $notification->id) }}" method="POST" style="display: inline;">
                        @csrf
                        <button type="submit" class="btn btn-success">Phê duyệt</button>
                    </form>
                    <form action="{{ route('thongbao.reject', $notification->id) }}" method="POST" style="display: inline;">
                        @csrf
