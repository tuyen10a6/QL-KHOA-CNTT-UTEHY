...
<body>
    ...
    <nav class="navbar" style="position: relative;">
        ...
        <div style="position: relative; display: inline-block; margin-top: 10px">
            ...
            <ul id="nganhDropdown" style="display: none; position: absolute; background: white; border: 1px solid #ccc; padding: 10px; list-style: none; z-index: 1000; width: 250px; border-radius: 20px">
                ...
                <table class="score-table">
                    ...
                    <tbody>
                        @foreach($dangKyMonHoc as $dk)
                            <tr>
                                <td class="score-value">{{ $dk->monHoc->ma_mon_hoc }}</td>
                                <td class="score-value">{{ $dk->monHoc->ten_mon_hoc }}</td>
                                <td class="score-value {{ !$dk->diem || !$dk->diem->diem_giua_ky ? 'empty' : '' }}">
                                    {{ $dk->diem ? $dk->diem->diem_giua_ky : 'Chưa có' }}
                                </td>
                                <td class="score-value {{ !$dk->diem || !$dk->diem->diem_cuoi_ky ? 'empty' : '' }}">
                                    {{ $dk->diem ? $dk->diem->diem_cuoi_ky : 'Chưa có' }}
                                </td>
                                <td class="score-value {{ !$dk->diem || !$dk->diem->diem_tong_ket ? 'empty' : '' }}">
                                    {{ $dk->diem ? $dk->diem->diem_tong_ket : 'Chưa có' }}
                                </td>
                                <td class="score-value {{ !$dk->diem || !$dk->diem->nhan_xet ? 'empty' : '' }}">
                                    {{ $dk->diem ? $dk->diem->nhan_xet : 'Chưa có' }}
                                </td>
                            </tr>
                        @endforeach
                    </tbody>
